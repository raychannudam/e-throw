...
class ATA_WC_Variation_Swatches_Frontend {
	...
	public function get_swatch_html( $html, $args ) {
		...
		if( $i > 15 ){
			$html       .= '<div style="clear:both;font-size:11px;">15 Swatches limit for each variation, Please <a href="https://athemeart.com/downloads/smart-variation-swatches-woocommerce-pro/" target="_blank"  style="color:#F00"> upgrade pro</a></div>';	
		}
		return $html;
	}
	...
	public function swatch_html( $html, $term, $attr, $args, $swatch_meta_options = array() ) {
		...
		switch ( $type ) {
			...
			case 'image':
				...
				$image = $image ? $image[0] : WC()->plugin_url() . '/assets/images/placeholder.png';
				$html  = sprintf(
					'<span class="swatch swatch-image swatch-%s %s %s %s %s" title="%s" data-value="%s"  style="width:%spx; height:%spx;"><img src="%s" alt="%s"></span>',
					esc_attr( $term->slug ),
					$selected,
					$style,
					$active,
