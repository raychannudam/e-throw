...
if ( ! class_exists( 'Smart_Filter_Widget' ) ) 
{
	...
	class Smart_Filter_Widget extends WPH_Widget
	{
		...
		function widget( $args, $instance )
		{
			...
			$out .= '<ul class="smart_attribute '.$attribute->attribute_type.'">';
			$i = 0 ;
			foreach( get_terms( 'pa_'.$attribute->attribute_name ) as $term){
			...
			$out       .= '<br/><div style="clear:both;font-size:11px;">8 Swatches limit for each variation, Please <a href="https://athemeart.com/downloads/smart-variation-swatches-woocommerce-pro/" target="_blank"  style="color:#F00"> upgrade pro</a></div>';	
			}
